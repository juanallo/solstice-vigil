...
const VIEWPORT = { width: 1800, height: 1236 };
...
async function buttonCenter(testId) {
  const el = page.getByTestId(testId);
  const box = await el.boundingBox();
  if (!box) return null;
  return { x: box.x + box.width / 2, y: box.y + box.height / 2, testId };
}
...
// 3. Intro rare encounter discovery
...
// 5–7. Neutral play toward identity reveal
for (let i = 0; i < 3; i++) {
...
// 9. Mid-game with identity badge
await capture('identity-badge', { choiceIdx: 0 });

// 10–13. Push toward Long Day ending
for (let i = 0; i < 6; i++) {
  const isOver = await page.getByTestId('gameover-screen').isVisible().catch(() => false);
  if (isOver) break;
...
console.log(`\nCaptured ${frames.length} frames → metadata.json`);
